var page = new Page("/Setting/IssueTypes");
...
$(function () {
    ...
    $(document).on('click', '#btn-new-save', function () {
        ...
        var validation = page.validate('#bdy-new', 'IssueType');

        if (validation.Valid) {
            page.save(validation.Entity, '/Save', $('#btn-new-save'), 'Issue Type Added', 'Save failed', done);
        } else {
    ...
    $(document).on('click', '#btn-edit-save', function () {
        ...
        var validation = page.validate('#bdy-edit', 'IssueType');

        if (validation.Valid) {
            page.save(validation.Entity, '/Update', $('#btn-edit-save'), 'Issue Type Updated', 'Update failed', done);
        } else {
            notify('<strong>Complete all the required fields:</strong><br /><br />' + validation.Message.join('<br />'), 'error')
        }
...
var deleteIssueType = function (id) {
    ...
    page.get('/Delete?id=' + id).then(function () {
        notify('Issue Type deleted', 'success');
        done();
    });
