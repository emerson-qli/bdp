var page = new Page("/Setting/AuditCategory");
...
$(function () {
    ...
    $(document).on('click', '#btn-new-save', function () {
        ...
        var validation = page.validate('#bdy-new', 'AuditCategory');

        if (validation.Valid) {
            page.save(validation.Entity, '/Save', $('#btn-new-save'), 'Audit Category Added', 'Save failed', done);
        } else {
    ...
    $(document).on('click', '#btn-edit-save', function () {
        ...
        var validation = page.validate('#bdy-edit', 'AuditCategory');

        if (validation.Valid) {
            page.save(validation.Entity, '/Update', $('#btn-edit-save'), 'Audit Category Updated', 'Update failed', done);
        } else {
            notify('<strong>Complete all the required fields:</strong><br /><br />' + validation.Message.join('<br />'), 'error')
        }
