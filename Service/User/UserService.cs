...
namespace Service.User {
    public class UserService : BaseService<UserRepository> {
        ...
        public List<Domain.DTO.UserEmployeeDTO> GetAllRegisteredUsers() {


            var entities = Repository().AllIncluding(a => a.Role)
                                       .ToList();

            var userEmployees = new List<Domain.DTO.UserEmployeeDTO>();


            foreach(var entity in entities) {

                var employee = new EmployeeService().GetAllBy(a => a.UserId == entity.Id).FirstOrDefault();
                if(employee == null) {
                    employee = new Domain.Models.Employee();
                }

                userEmployees.Add(new Domain.DTO.UserEmployeeDTO {
                    Email           = entity.Username,
                    UserId          = entity.Id,
                    UserLastLogin   = entity.LastLogin,
